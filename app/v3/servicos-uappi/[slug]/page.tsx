...
export default async function UappiProductPage({ params }: Props) {
  ...
  return (
    <>
      ...
      <section className="bg-white py-10 sm:py-16">
        <div className="mx-auto grid max-w-[1440px] gap-8 px-5 sm:px-10 lg:grid-cols-[1.08fr_.92fr] lg:gap-16">
          ...
          <div className="flex flex-col py-2">
            ...
            <p className="mt-5 font-mono text-xs font-bold uppercase tracking-[.08em] text-gold-hover">Serviço Uappi · {service.keyword}</p>
      ...
      <section className="store-section bg-cream"><div className="mx-auto grid max-w-[1440px] gap-10 px-5 sm:px-10 lg:grid-cols-[.8fr_1.2fr]"><div><p className="font-mono text-xs font-bold uppercase tracking-[.08em] text-gold-hover">O que está incluso</p><h2 className="mt-3 font-display text-4xl font-bold leading-none tracking-[-.02em]">Uma solução pronta para sair da vitrine e gerar resultado.</h2></div><ul className="grid gap-4 sm:grid-cols-2">{service.deliverables.map((item) => <li key={item} className="flex gap-3 bg-white p-5 text-[1.05rem] leading-relaxed text-warm-gray"><Check className="mt-0.5 shrink-0 text-gold-hover" size={18} /><span>{item}</span></li>)}</ul></div></section>
      <ServiceBody service={service} />
      <section className="store-section bg-white"><div className="mx-auto max-w-[1440px] px-5 sm:px-10"><div className="store-section-heading"><div><p className="font-mono text-xs font-bold uppercase tracking-[.08em] text-gold-hover">Complete sua proposta</p><h2 className="mt-3">Quem leva este serviço também olha.</h2></div><Link href="/servicos-uappi" title="Voltar à vitrine Uappi" className="text-sm font-bold underline underline-offset-4 transition hover:text-[#006db1]">Voltar à vitrine Uappi</Link></div><div className="grid gap-5 md:grid-cols-3">{related.map((item) => <Link key={item.slug} href={`/servicos-uappi/${item.slug}`} title={item.shortTitle} className="group border border-soft-beige bg-white p-5 transition hover:-translate-y-1 hover:shadow-lg"><div className="relative aspect-[1.4] overflow-hidden bg-soft-beige"><Image src={item.image} alt="" fill className="object-cover transition duration-500 group-hover:scale-105" sizes="33vw" /></div><p className="mt-5 font-mono text-[.65rem] font-bold uppercase tracking-[.08em] text-warm-gray">Solução complementar</p><h3 className="mt-2 font-display text-2xl font-bold text-luxury-black">{item.shortTitle}</h3><span className="mt-5 inline-flex items-center gap-1 text-sm font-bold text-gold-hover">Ver serviço <ChevronRight size={15} /></span></Link>)}</div></div></section>
