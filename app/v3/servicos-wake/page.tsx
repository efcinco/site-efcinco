      ...
      <div className="mx-auto max-w-[1440px] px-5 sm:px-10">
        <div className="store-section-heading">
          <div>
            ...
            <p className="mt-5 font-mono text-xs font-bold uppercase tracking-[.16em] text-gold-hover">Vitrine Wake</p>
        ...
        <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {wakeServices.map((service) => (
            <Link key={service.slug} href={`/servicos-wake/${service.slug}`} title={service.shortTitle} className="store-product-card group">
              <div className="store-product-media" style={service.slug.includes("integracoes") ? { background: "transparent" } : undefined}><Image src={service.image} alt="" fill className={`${service.slug.includes("integracoes") ? "object-contain" : "object-cover"} transition duration-500 group-hover:scale-105`} sizes="(max-width: 1024px) 50vw, 33vw" /><span className="absolute left-4 top-4 rounded-full bg-white px-3 py-1 text-xs font-bold uppercase tracking-[.12em] text-luxury-black">Wake</span></div>
