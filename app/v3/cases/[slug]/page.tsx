    ...
    <>
      <section className="bg-luxury-black px-5 pb-16 pt-12 text-white sm:px-10">
        <div className="mx-auto grid max-w-[1440px] gap-10 lg:grid-cols-[.9fr_1.1fr]">
          <div className="flex flex-col justify-end"><Link href="/cases" title="Voltar aos cases" className="inline-flex items-center gap-2 text-sm font-bold text-gold transition hover:underline"><ArrowLeft size={16} /> Voltar aos cases</Link><p className="mt-12 font-mono text-xs font-bold uppercase tracking-[.08em] text-gold">{item.client} · {item.segment}</p><h1 className="mt-4 font-display text-5xl font-bold leading-[.95] tracking-[-.02em] sm:text-7xl">{item.title}</h1><p className="mt-6 text-lg leading-relaxed text-white/70">{item.summary}</p></div>
