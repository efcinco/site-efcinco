...
export default function IugisPage() {
  return (
    <>
      <section className="store-section overflow-hidden bg-ink text-white">
        <div className="mx-auto grid max-w-[1440px] items-center gap-10 px-5 sm:px-10 lg:grid-cols-[1fr_.85fr]">
          <div>
            ...
            <p className="eyebrow mt-8">Automações para e-commerce</p>
        ...
        <div className="mx-auto max-w-[1440px] px-5 sm:px-10">
          <div className="grid gap-12">
            {iugisServices.map((service, index) => (
              <article key={service.slug} className="grid overflow-hidden rounded-2xl bg-white shadow-sm lg:grid-cols-2">
                ...
                <div className="flex flex-col justify-center p-8 sm:p-12">
                  <p className="eyebrow">Automação Iugis</p>
                  <h2 className="mt-4 font-display text-4xl font-bold leading-none tracking-[-.02em] text-ink">{service.title}</h2>
                  <p className="mt-5 text-base leading-relaxed text-muted">{service.description}</p>
                  <ul className="mt-7 grid gap-3 text-sm text-muted">
                    {service.deliverables.slice(0, 3).map((item) => <li key={item} className="flex gap-2"><Check size={17} className="shrink-0 text-accent" />{item}</li>)}
                  </ul>
                  <div className="mt-8 flex flex-wrap gap-3">
                    <Link href={`/automacoes-iugis/${service.slug}`} title={`Ver automação: ${service.shortTitle}`} className="inline-flex items-center rounded-full bg-brand px-5 py-3 text-sm font-bold text-white transition hover:bg-brand-hover">Ver automação <ArrowRight size={16} className="ml-1.5" /></Link>
                    <Link href="/contato" title="Solicitar proposta" className="inline-flex items-center rounded-full border border-ink px-5 py-3 text-sm font-bold text-ink transition hover:bg-mist">Solicitar proposta</Link>
                  </div>
                </div>
              </article>
            ))}
