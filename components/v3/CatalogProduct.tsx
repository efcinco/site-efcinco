    ...
    <>
      ...
      <div className="border-b border-soft-beige bg-white">
        <div className="mx-auto flex max-w-[1440px] items-center gap-1 px-5 py-4 text-xs text-warm-gray sm:px-10">
          ...
          <Link href={catalogPath} title={catalogName} className="transition hover:text-[#006db1] hover:underline">{catalogName}</Link><ChevronRight size={13} />
        ...
        <div className="mx-auto grid max-w-[1440px] gap-10 px-5 sm:px-10 lg:grid-cols-[.8fr_1.2fr]">
          ...
          <ul className="grid gap-4 sm:grid-cols-2">
            {service.deliverables.map((item) => (
              <li key={item} className="flex gap-3 bg-white p-5 text-[1.05rem] leading-relaxed text-warm-gray">
                <Check className="mt-0.5 shrink-0 text-gold-hover" size={18} /><span>{item}</span>
              </li>
            ))}
          </ul>
        </div>
      ...
      <section className="store-section bg-white">
        <div className="mx-auto max-w-[1440px] px-5 sm:px-10">
          ...
          <div className="grid gap-5 md:grid-cols-3">
            {related.map((item) => (
              <Link key={item.slug} href={`${catalogPath}/${item.slug}`} title={item.shortTitle} className="group border border-soft-beige bg-white p-5 transition hover:-translate-y-1 hover:shadow-lg">
